...
fn update_constellations(constellations: Vec<Constellation>, point: Point) -> Vec<Constellation> {
    ...
    for constellation in constellations {
        if in_constellation(&constellation, &point) {
            found_constellation =
                update_found_constellation(found_constellation, constellation, point);
        } else {
            result.push(constellation);
        }
    }
    match found_constellation {
        None => result.push(Constellation {
            points: vec![point],
        }),
...
fn update_found_constellation(
    ...
    let constellation_plus_point = Constellation { points: new_points };
    match found_constellation {
        None => Some(constellation_plus_point),
        Some(constellation) => {
            /*
            ...
            */
            let merged = merge_constellations(constellation_plus_point, constellation);
            Some(merged)
        }
    }
}
...
fn distance(p1: &Point, p2: &Point) -> i32 {
    (p1.x - p2.x).abs() + (p1.y - p2.y).abs() + (p1.z - p2.z).abs() + (p1.t - p2.t).abs()
}
...
mod tests {
    ...
    fn check_if_example_0_works() {
        let example = vec![
            0, 0, 0, 0, 3, 0, 0, 0, 0, 3, 0, 0, 0, 0, 3, 0, 0, 0, 0, 3, 0, 0, 0, 6, 9, 0, 0, 0, 12,
            0, 0, 0,
        ];
    ...
    fn check_if_example_1_works() {
        let example_1 = vec![
            -1, 2, 2, 0, 0, 0, 2, -2, 0, 0, 0, -2, -1, 2, 0, 0, -2, -2, -2, 2, 3, 0, 2, -1, -1, 3,
            2, 2, -1, 0, -1, 0, 0, 2, 1, -2, 3, 0, 0, 0,
        ];
    ...
    fn check_if_example_2_works() {
        let example_2 = vec![
            1, -1, 0, 1, 2, 0, -1, 0, 3, 2, -1, 0, 0, 0, 3, 1, 0, 0, -1, -1, 2, 3, -2, 0, -2, 2, 0,
            0, 2, -2, 0, -1, 1, -1, 0, -1, 3, 2, 0, 2,
        ];
    ...
    fn check_if_example_3_works() {
        let example_3 = vec![
            1, -1, -1, -2, -2, -2, 0, 1, 0, 2, 1, 3, -2, 3, -2, 1, 0, 2, 3, -2, -1, -1, 1, -2, 0,
            -2, -1, 0, -2, 2, 3, -1, 1, 2, 2, 0, -1, -2, 0, -2,
        ];
